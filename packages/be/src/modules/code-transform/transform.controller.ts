...
不要编造答案，只使用提供的参考信息。`
  ...
  async transformCode(@Body() body: { oldCode: string }) {
    ...
    const similarExamples = await this.ragSystem.similaritySearch(oldCode);

    return { similarExamples };

    // Use examples as context for the LLM
...
3. 将URL字符串转换为this.$util.openWin/openModal的结构化对象
4. 使用e.colDef.field代替field字符串比较
5. 使用现代ES6+语法
6. 保持业务逻辑不变

**重要提示：请返回完整的代码，不要省略任何部分，不要使用省略号(...)。生成的代码必须是可以直接运行的。**

参考以下类似的例子：
${similarExamples.map((item: any) => item.content).join('\n\n')}
`
      }),
      new HumanMessage({
    ...
    console.log(similarExamples, 'similarExamples');
